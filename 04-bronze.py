...
class Bronze():
    def __init__(self):
        ...
        self.checkpoint = Conf.base_dir_checkpoint + "/checkpoints"
    ...
    def consume_user_registration(self, once=True, processing_time = "5 seconds"):
        ...
        stream_writer = (df_stream.writeStream
                            .format("delta")
                            .option("checkpointLocation", self.checkpoint+"/registered_users_bz")
                            .outputMode("append")
        ...
        if once == True:
            return stream_writer.trigger(availableNow=True).toTable(f"{self.db_name}.registered_users_bz")
        else:
            return stream_writer.trigger(processingTime = processing_time).toTable(f"{self.db_name}.registered_users_bz")
    ...
    def consume_gym_logins(self, once=True, processing_time = "5 seconds"):
        ...
        stream_writer = (df_stream.writeStream
                            .format("delta")
                            .option("checkpointLocation", self.checkpoint+"/gym_logins_bz")
                            .outputMode("append")
        ...
        if once == True:
            return stream_writer.trigger(availableNow=True).toTable(f"{self.db_name}.gym_logins_bz")
        else:
            return stream_writer.trigger(processingTime = processing_time).toTable(f"{self.db_name}.gym_logins_bz")
    ...
    def consume_kafka_multiplex(self, once=True, processing_time="5 seconds"):
        ...
        df_date_lookup = spark.table(f"{self.db_name}.date_lookup").select("date", "week_part")
        ...
        df_stream = (spark.readStream
                        ...
                        .withColumn("load_time", F.current_timestamp())
                        .withColumn("source_file", F.input_file_name())
                        .join(F.broadcast(df_date_lookup), [F.to_date(F.col("timestamp")/1000).cast("timestamp") == F.col("date")], "left")
                        )
        
        stream_writer = (df_stream.writeStream
                            .format("delta")
                            .option("checkpointLocation", self.checkpoint + "/kafka_multiplex_bz")
                            .outputMode("append")
        ...
        if once == True:
            return stream_writer.trigger(availableNow=True).toTable(f"{self.db_name}.kafka_multiplex_bz")
        else:
            return stream_writer.trigger(processingTime=processing_time).toTable(f"{self.db_name}.kafka_multiplex_bz")
    ...
    def assert_count(self, table_name, expected_count, filter="true"):
        ...
        actual_count = spark.read.table(f"hive_metastore.{self.db_name}.{table_name}").where(filter).count()
