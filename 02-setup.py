...
class SetupHelper():   
    ...
    def create_db(self):
        ...
        # Explicitly set hive_metastore as catalog
        spark.sql(f"CREATE DATABASE IF NOT EXISTS hive_metastore.{self.db_name}")
        spark.sql(f"USE hive_metastore.{self.db_name}")
    ...
    def create_registered_users_bz(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.registered_users_bz(
                    user_id long,
                    device_id long, 
                    mac_address string, 
    ...
    def create_gym_logins_bz(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE OR REPLACE TABLE {self.db_name}.gym_logins_bz(
                    mac_address string,
                    gym bigint,
                    login double,                      
                    logout double,                    
    ...
    def create_kafka_multiplex_bz(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.kafka_multiplex_bz(
                  key string, 
                  value string, 
                  topic string, 
                  partition bigint, 
    ...
    def create_users(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE OR REPLACE TABLE {self.db_name}.users(
                    user_id bigint, 
                    device_id bigint, 
                    mac_address string,
    ...
    def create_gym_logs(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE OR REPLACE TABLE {self.db_name}.gym_logs(
                    mac_address string,
                    gym bigint,
                    login timestamp,                      
                    logout timestamp
    ...
    def create_user_profile(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.user_profile(
                    user_id bigint, 
                    dob DATE, 
                    sex STRING, 
                    gender STRING, 
    ...
    def create_heart_rate(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.heart_rate(
                    device_id LONG, 
                    time TIMESTAMP, 
                    heartrate DOUBLE, 
                    valid BOOLEAN)
    ...
    def create_user_bins(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.user_bins(
                    user_id BIGINT, 
                    age STRING, 
                    gender STRING, 
                    city STRING, 
    ...
    def create_workouts(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.workouts(
                    user_id INT, 
                    workout_id INT, 
                    time TIMESTAMP, 
                    action STRING, 
    ...
    def create_completed_workouts(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.completed_workouts(
                    user_id INT, 
                    workout_id INT, 
                    session_id INT, 
    ...
    def create_workout_bpm(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.workout_bpm(
                    user_id INT, 
                    workout_id INT, 
                    session_id INT,
                    start_time TIMESTAMP, 
    ...
    def create_date_lookup(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.date_lookup(
                    date date, 
                    week int, 
                    year int, 
                    month int, 
    ...
    def create_workout_bpm_summary(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE TABLE IF NOT EXISTS {self.db_name}.workout_bpm_summary(
                    workout_id INT, 
                    session_id INT, 
                    user_id BIGINT, 
                    age STRING, 
    ...
    def create_gym_summary(self):
        if(self.initialized):
            ...
            spark.sql(f"""CREATE OR REPLACE VIEW {self.db_name}.gym_summary AS
    ...
    def assert_table(self, table_name):
        assert spark.sql(f"SHOW TABLES IN hive_metastore.{self.db_name}") \
    ...
    def validate(self):
        ...
        assert spark.sql(f"SHOW DATABASES IN hive_metastore") \
    ...
    def cleanup(self): 
        if spark.sql(f"SHOW DATABASES IN hive_metastore").filter(f"databaseName == '{self.db_name}'").count() == 1:
            print(f"Dropping the database {self.db_name}...", end='')
            spark.sql(f"DROP DATABASE hive_metastore.{self.db_name} CASCADE")
            print("Done")
