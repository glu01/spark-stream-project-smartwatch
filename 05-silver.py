...
class Silver:
    ...
    def upsert_users(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        query = f"""
            MERGE INTO {self.db_name}.users a
            USING users_delta b
        ...
        df_delta = (
            ...
            .table(f"{self.db_name}.registered_users_bz")
            .selectExpr(
                "user_id",
                "device_id",
                "mac_address",
                "cast(registration_timestamp as timestamp)",
            )
        ...
        stream_writer = (
            df_delta.writeStream.foreachBatch(data_upserter.upsert)
            .outputMode("update")
    ...
    def upsert_gym_logs(
        self, once=True, processing_time="15 seconds", startingVersion=0
    ):
        from pyspark.sql import functions as F
        ...
        query = f"""
            MERGE INTO {self.db_name}.gym_logs a
        ...
        df_delta = (
            ...
            # .option("withEventTimeOrder", "true")
            # .option("maxFilesPerTrigger", self.maxFilesPerTrigger)
            .table(f"{self.db_name}.gym_logins_bz")
            .selectExpr(
                "mac_address",
                "gym",
                "cast(login as timestamp)",
                "cast(logout as timestamp)",
            )
    ...
    def upsert_user_profile(
        self, once=False, processing_time="15 seconds", startingVersion=0
    ):
        from pyspark.sql import functions as F
        ...
        query = f"""
            MERGE INTO {self.db_name}.user_profile a
        ...
        df_cdc = (
            ...
            # .option("withEventTimeOrder", "true")
            # .option("maxFilesPerTrigger", self.maxFilesPerTrigger)
            .table(f"{self.db_name}.kafka_multiplex_bz")
        ...
        stream_writer = (
            df_cdc.writeStream.foreachBatch(data_upserter.upsert)
            .outputMode("update")
    ...
    def upsert_workouts(
        self, once=False, processing_time="10 seconds", startingVersion=0
    ):
        from pyspark.sql import functions as F
        ...
        query = f"""
            MERGE INTO {self.db_name}.workouts a
    ...
    def upsert_heart_rate(
        ...
        query = f"""
        MERGE INTO {self.db_name}.heart_rate a
        ...
        df_delta = (
            ...
            .select(
                "v.*",
                F.when(F.col("v.heartrate") <= 0, False).otherwise(True).alias("valid"),
            )
            .withWatermark("time", "30 seconds")
    ...
    def upsert_user_bins(
        ...
        query = f"""
            MERGE INTO {self.db_name}.user_bins a
        ...
        df_user = spark.table(f"{self.db_name}.users").select("user_id")
        ...
        df_delta = (
            ...
            # .option("withEventTimeOrder", "true")
            # .option("maxFilesPerTrigger", self.maxFilesPerTrigger)
            .table(f"{self.db_name}.user_profile")
    ...
    def upsert_completed_workouts(
        ...
        query = f"""
        MERGE INTO {self.db_name}.completed_workouts a
        ...
        df_delta = df_start.join(df_stop, join_condition).select(
            df_start.user_id,
            df_start.workout_id,
            df_start.session_id,
            df_start.start_time,
    ...
    def upsert_workout_bpm(
        ...
        # Idempotent - Only one user workout session completes. So ignore the duplicates and insert the new records
        query = f"""
        MERGE INTO {self.db_name}.workout_bpm a
        ...
        df_completed_workouts = (
            ...
            # .option("withEventTimeOrder", "true")
            # .option("maxFilesPerTrigger", self.maxFilesPerTrigger)
            .table(f"{self.db_name}.completed_workouts")
        ...
        df_bpm = (
            ...
            # .option("withEventTimeOrder", "true")
            # .option("maxFilesPerTrigger", self.maxFilesPerTrigger)
            .table(f"{self.db_name}.heart_rate")
            .filter("valid = True")
        ...
        df_delta = df_bpm.join(df_completed_workouts, join_condition).select(
            "user_id",
            "workout_id",
            "session_id",
            "start_time",
    ...
    def assert_count(self, table_name, expected_count, filter="true"):
        ...
        actual_count = (
            spark.read.table(f"hive_metastore.{self.db_name}.{table_name}")
            .where(filter)
            .count()
        )
        assert (
        ...
        print(
            f"Found {actual_count:,} / Expected {expected_count:,} records where {filter}: Success"
        )
    ...
    def validate(self, sets):
        ...
        print(
            f"Silver layer validation completed in {int(time.time()) - start} seconds"
        )
